...
//				}
				...
				Future <StringBuilder> future = executor.submit (
					() -> HttpClientUtils.post (url, buffer.toString (), GATEWAY_OPTION_HTTP_POST));
				
				future.addListener (( f ) -> {
					if (f.isSuccess ()) {
						respone = ( ( StringBuilder ) f.get (GATEWAY_OPTION_HTTP_POST,
			...
			if (gateway.getServerPath ().equals (uri)) {
				for ( RouteAttribute route : RoutingLoader.ROUTERS ) {
					if (route.getServerPath ().equals (uri)) {
						String[] keys = StringUtils.delimitedListToStringArray (route.getKeyWord (),
																				GATEWAY_OPTION_KEY_WORD_SPLIT);
						boolean match = true;
						for ( String key : keys ) {
							if (key.isEmpty ())
								continue;
	...
	private void writeResponse (
		StringBuilder respone, HttpObject current, ChannelHandlerContext ctx ) {
		
		if (respone != null) {
			boolean keepAlive = HttpUtil.isKeepAlive (request);
			
			FullHttpResponse response = new DefaultFullHttpResponse (HTTP_1_1, current == null ?
				OK :
				current.decoderResult ().isSuccess () ? OK : BAD_REQUEST, Unpooled.copiedBuffer (
				respone.toString (), GATEWAY_OPTION_CHARSET));
